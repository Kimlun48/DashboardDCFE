...
function KaliurangDelivCustLateDetail() {
    document.title = "Report-DetailKaliurangDelivCustScheduel";

    const [detaildelivcust, setDetailDelivCust] = useState([]);
    ...
    const fetchData = async () => {
        try {
            const response = await Api.get('api/kaliurangdelivcusstorestatisticlatedetail');
            const data = response.data.data; 
            setDetailDelivCust(data);
    ...
    useEffect(() => {
        if (Array.isArray(detaildelivcust)) { // Ensure detailbinin is an array
            const lowercasedSearch = search.toLowerCase();
            const filtered = detaildelivcust.filter(item =>
                    ...
                    <div className="col-md-12">
                        <div className="card border-0 rounded shadow-sm border-top-success">
                            <div className="card-header d-flex justify-content-between align-items-center">
                                <div>
                                    <span className="font-weight-bold">Store - Detail Delivery Customer Late </span>
                                </div>
                            </div>
...
export default KaliurangDelivCustLateDetail;
