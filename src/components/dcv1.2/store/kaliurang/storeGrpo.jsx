...
const ChartStoreGrpo = () => {
    ...
    //     queryKey: ['grpoData'],  // Unique key for the query
    //     queryFn: fetchGrpoData,
    //     refetchInterval: time,  // Refetch data every `time` ms
    // });
    const { data, error, isLoading } = useQuery({
        queryKey: ['grpoData'],
        queryFn: fetchGrpoData,
        ...
        refetchInterval: time,  // Refetch data every 5 minutes
    });
    ...
    // Ensure data is 0 if not provided
    const chartData = [
        { name: 'Bin IN Store', value: data.ONHANDIN || 0 },
        { name: 'Bin OUT Store', value: data.ONHANDOUT || 0 },
        { name: 'Bin Transit Store', value: data.ONHANDTRANSIT || 0 },
    ];
    ...
    // Ensure dataMax is at least 1 to avoid rendering issues
    const dataMax = Math.max(1, ...chartData.map(item => item.value));
    ...
    return (
        <React.Fragment>
            ...
            <ResponsiveContainer width="100%" height={250}>
                <BarChart layout="vertical" data={chartData} margin={{ top: 5, right: 10, left: 5, bottom: 5 }}>
                    <defs>
                        <linearGradient id="colorBinINStore" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="0%" stopColor="#01FEFF" stopOpacity={1}/>
                            <stop offset="100%" stopColor="#01FEFF" stopOpacity={0.5}/>
